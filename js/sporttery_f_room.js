...
//Game switch fn
var game_ary = Array("胜平负","让球胜平负","比分","总进球","全半场","混合过关","2选1");
//Define Initial Game
...
//平分比 Lightbox
	...
	//混合過關
		...
		$("#sporttery_f_room5 .sportf_game").each(function(i){
			var tg = $(this);
			...
			$(this).find('.sportf_detail_lightbox').on('click',function(){
				...
				$("#sporttery_f_room5 .sportf_game").eq(i).find('.sportf_bd4').each(function(ai){
					if($(this).hasClass("sportf_down")){
				...
				var refval4 = tg.find(".sportf_mmn_refnum4").refValTransfer();//Get persont value [4]
				...
				var smmnv4 = $(".sportf_mmn4").eq(i).val().split(',');
				...
				$(".sprotf_detail_4").find(".sportf_bd4_2_2").removeClass('on');
				...
				$(".sprotf_detail_4").put_select_val_to_lightbox_column(smmnv4);
			})
		})// end 混合過關
	...
	//Check and Close Lightbox
		...
		$(".sportf_detail_checkbtn").on('click',function(){
			...
			$("#sporttery_f_room5 .sportf_game .sport_tbs").eq(ny).find('.sportf_bdn').each(function(i){
				$(this).removeClass('on');
